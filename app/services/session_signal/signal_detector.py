...
hold_loss: Curls falling limp, styles not lasting, or hair losing shape and definition over time. Focuses on structure, elasticity, and style longevity.
  ...
  DO NOT flag for: anything degrading over time other than the curl itself. What must get worse is the SHAPE, DEFINITION, VOLUME, or HOLD of the curl. A day count alone is not enough: "by day 3 it is dry" is absorption_blocked, "by day 3 my scalp hurts" is scalp_sensitivity, and neither is hold_loss. Also do not flag frizz in one area while the style holds: "my wash and go lasts a week but I get frizz at the back top" is not hold_loss, because the style lasted. Ask one question before you flag: did the curl lose its shape? If the user says the style lasted, or names a problem that is not the curl shape, do not flag.
